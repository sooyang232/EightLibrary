...
public List getArticles(int start,int end) {
		...
		try {
			 ...
			 */
			sql="select * from b2 order by b2_num desc,b2_reply asc limit ?,?";
			pstmt=con.prepareStatement(sql);
		...
		try {
			...
			if(search==null || search=="") { // 검색분야 선택X
				...
				if(search.equals("subject_content")) {	//제목+본문
					sql="select count(*) from b2 where subject like '%"+
				searchtext+"%' or content like '%"+searchtext+"%'";
				}else {	//제목,작성자 -> 매개변수를 이용해서 하나의 sql통합
					sql="select count(*) from b2 where "+search+" like '%"+
				searchtext+"%'";
				}
			}
		...
		public List getBoardArticles(int start, int end, String search, String searchtext) {// getMemberList(int start,int
			...
			try {
				...
				if (search == null | search == "") {
					sql = "select * from b2 order by b2_num desc,b2_reply asc limit ?,?";// 1,10
				} else { // 제목+본문
					if (search.equals("title_content")) { // 제목+본문
						sql = "select * from b2 where b2_title like '%" +searchtext+ "%' or b2_content like '%"
								+searchtext+ "%' order by b2_num desc,b2_reply asc limit ?,?";
					} else { // 제목,작성자 -> 매개변수를 이용해서 하나의 sql통합
						sql = "select * from b2 where " +search+ " like '%" +searchtext
								+ "%' order by b2_num desc,b2_reply asc limit ?,?";
					}
				}
